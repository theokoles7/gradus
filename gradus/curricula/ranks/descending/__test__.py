"""# gradus.curricula.ranks.descending.test

Test suite for descending curriculum rank.
"""
...
def descending_indices(
    ...
    """Instantiate Descending Rank with a Fresh Cache Directory and Return Indices."""
    return Descending(
        metric =        metric,
...
class TestDescendingReturnType:
    """# Return Type Tests"""
    ...
    def test_returns_list(self,
        ...
        """Descending Rank Should Return a List."""
        # Load indices.
        indices:    List[int] = descending_indices(synthetic_scores, "saturation-time", tmp_path)

        # Ensure indices is a list.
        assert isinstance(indices, list), "Descending rank should return a list of indices."
...
class TestDescendingCompleteness:
    """# Completeness Tests
...
class TestDescendingOrder:
    ...
    non-increasing from first to last index.
    """

    def test_metric_values_nonincreasing(self,
        synthetic_scores:   DataFrame,
        tmp_path:           TempPathFactory
    ) -> None:
        """Metric Values at Ranked Indices Should be Non-Increasing."""
        # Load indices.
        indices:    List[int] = descending_indices(synthetic_scores, "saturation-time", tmp_path)
    ...
    def test_first_index_has_maximum_value(self,
        synthetic_scores:   DataFrame,
        tmp_path:           TempPathFactory
    ) -> None:
        """First Ranked Index Should Correspond to the Maximum Metric Value."""
        # Load indices.
        indices:        List[int] = descending_indices(synthetic_scores, "saturation-time", tmp_path)
        ...
        # Assert that the first value is the highest.
        assert  first_value == approx(max_value),                       \
                f"First ranked sample has value {first_value:.4f}, "    \
                f"but maximum is {max_value:.4f}."

    def test_last_index_has_minimum_value(self,
        synthetic_scores:   DataFrame,
        tmp_path:           TempPathFactory
    ) -> None:
        """Last Ranked Index Should Correspond to the Minimum Metric Value."""
        # Load indices.
        indices:    List[int] = descending_indices(synthetic_scores, "saturation-time", tmp_path)
        ...
        # Assert that the last value is the lowest.
        assert  last_value == approx(min_value),                        \
    ...
    def test_ordering_consistent_across_metrics(self,
        ...
        for metric in ["saturation-time", "color-variance", "edge-density"]:
            ...
            # Load indices.
            indices:    List[int] = descending_indices(synthetic_scores, metric, tmp_path / metric)
    ...
    def test_descending_is_reverse_of_ascending(self,
        ...
        """Descending Order Should be the Exact Reverse of Ascending Order."""
        from gradus.curricula.ranks.ascending import Ascending

        # Load ascending indices.
        asc:    List[int] = Ascending(
        ...
        # Load descending indices.
        desc:   List[int] = descending_indices(synthetic_scores, "saturation-time", tmp_path)

        # Assert that descending is the reverse of ascending.
        assert  desc == asc[::-1], \
                "Descending rank should be the exact reverse of ascending rank."


class TestDescendingCaching:
    """# Cache Tests
    ...
    def test_cache_file_created(self,
        ...
        # Construct descending indices according to saturation time.
        descending_indices(synthetic_scores, "saturation-time", tmp_path)
    ...
    def test_cached_result_matches_original(self,
        ...
        first_run:  List[int] = descending_indices(synthetic_scores, "saturation-time", tmp_path)
        second_run: List[int] = descending_indices(synthetic_scores, "saturation-time", tmp_path)
    ...
    def test_different_metrics_produce_different_cache_keys(self,
        ...
        descending_indices(synthetic_scores, "saturation-time", tmp_path)
        descending_indices(synthetic_scores, "color-variance",  tmp_path)
...
class TestDescendingValidation:
    """# Validation Tests"""
    ...
    def test_rejects_multiple_metrics(self,
        ...
        """Descending Rank Should Reject More Than One Metric."""
