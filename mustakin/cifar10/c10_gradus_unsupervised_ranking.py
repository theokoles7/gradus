...
def compute_all_complexity_features(dataset, device, max_samples=None):
    ...
    raw_dataset = datasets.CIFAR10(root='./data', train=True, download=True, transform=raw_transform)
    ...
    for col in features_normalized.columns:
        ...
        if col_max - col_min > 1e-10:
            features_normalized[col] = (features_normalized[col] - col_min) / (col_max - col_min)
        else:
            features_normalized[col] = 0.0
    # Invert compression_ratio: high ratio = simple -> low normalized = simple
    features_normalized['compression_ratio'] = 1.0 - features_normalized['compression_ratio']
...
def rank_images_unsupervised(features_df, train_indices):
    ...
    """
    metric_cols = ['color_variance', 'compression_ratio', 'edge_density',
                   'spatial_frequency', 'wavelet_energy', 'wavelet_entropy',
                   'edge_object_count']
    ...
    for i in range(n):
        if i % 5000 == 0:
            ...
            if i > 0:
                rate = i / elapsed
                remaining = (n - i) / rate
                print(f"  Ranking: image {i}/{n} ({100*i/n:.1f}%) "
...
class CurriculumSampler(Sampler):
    ...
    """
    def __init__(self, sorted_indices, total_epochs, start_fraction=0.3, full_data_epoch=None):
        self.sorted_indices = np.array(sorted_indices)
        self.total_epochs = total_epochs
        self.start_fraction = start_fraction
        self.full_data_epoch = full_data_epoch if full_data_epoch is not None else int(0.6 * total_epochs)
...
def train_epoch(model, device, train_loader, optimizer, epoch, scheduler=None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % 100 == 0:
            print(f'  Epoch {epoch} [{total_images}/{num_batches * data.size(0)} '
    ...
    print(f"  Epoch {epoch} done in {epoch_time:.1f}s | Loss: {epoch_loss:.4f} | "
...
def plot_complexity_distribution(features_df, save_path='complexity_distribution_gradus.pdf'):
    """Plot distribution of complexity features and combined score."""
    fig, axes = plt.subplots(2, 4, figsize=(24, 10))

    cols = ['color_variance', 'compression_ratio', 'edge_density', 'spatial_frequency',
            'wavelet_energy', 'wavelet_entropy', 'edge_object_count', 'complexity_score']
    titles = ['Color Variance', 'Compression Ratio', 'Edge Density', 'Spatial Frequency',
              'Wavelet Energy', 'Wavelet Entropy', 'Edge Object Count', 'Combined Complexity Score']
    colors = ['#2196F3', '#4CAF50', '#FF9800', '#E91E63', '#9C27B0', '#00BCD4', '#795548', '#F44336']

    for ax, col, title, color in zip(axes.flatten(), cols, titles, colors):
        ax.hist(features_df[col], bins=50, alpha=0.7, color=color, edgecolor='black', linewidth=0.5)
        ax.set_title(title, fontsize=12, fontweight='bold')
        ax.set_xlabel('Value')
        ax.set_ylabel('Count')
        ax.axvline(features_df[col].mean(), color='red', linestyle='--', linewidth=1.5, label=f'Mean: {features_df[col].mean():.3f}')
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)

    plt.suptitle('Image Complexity Feature Distributions (CIFAR-10 Training Set)', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def plot_curriculum_schedule(total_epochs, start_fraction, full_data_epoch, total_samples,
                            samples_per_epoch, save_path='curriculum_schedule_gradus.pdf'):
    ...
    axes[0].axvline(full_data_epoch, color='red', linestyle='--', alpha=0.5, label=f'Full data at epoch {full_data_epoch}')
    ...
    axes[1].axhline(total_samples, color='gray', linestyle='--', alpha=0.5, label=f'Total: {total_samples}')
...
def plot_training_metrics(epoch_times, train_losses, train_accs, val_losses, val_accs,
                         save_path='training_metrics_curriculum_gradus.pdf'):
...
def plot_confusion_matrix(all_targets, all_preds, class_names, title='Confusion Matrix',
                         save_path='confusion_matrix_curriculum_gradus.pdf'):
...
def plot_comparison(results_t2b, results_b2t, save_path='comparison_gradus.pdf'):
    ...
    # Training Loss
    axes[0].plot(epochs, results_t2b['train_losses'], label='Top-to-Bottom', linewidth=1.5)
    axes[0].plot(epochs, results_b2t['train_losses'], label='Bottom-to-Top', linewidth=1.5)
    ...
    # Training Accuracy
    axes[2].plot(epochs, results_t2b['train_accs'], label='Top-to-Bottom', linewidth=1.5)
    axes[2].plot(epochs, results_b2t['train_accs'], label='Bottom-to-Top', linewidth=1.5)
    ...
    plt.suptitle('Unsupervised Ranking: Top-to-Bottom vs Bottom-to-Top Curriculum',
...
def run_single_experiment(direction_label, curriculum_order, full_train_dataset,
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        print(f"Curriculum: using {num_samples_this_epoch}/{total_train_samples} samples ({fraction_used:.1f}%)")
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), f'best_resnet18_{direction_label}_gradus.pth')
    ...
    final_val_loss, final_val_acc, _, _ = validate(model, device, val_loader, dataset_name="Validation")
    ...
    plot_training_metrics(epoch_times, train_losses, train_accs, val_losses, val_accs,
                         save_path=f'training_metrics_{direction_label}_gradus.pdf')
    plot_confusion_matrix(test_targets, test_preds, class_names,
                         title=f'Test Confusion Matrix ({direction_label})',
                         save_path=f'confusion_matrix_{direction_label}_gradus.pdf')
    plot_curriculum_schedule(total_epochs, start_fraction, full_data_epoch,
                           total_train_samples, samples_per_epoch,
                           save_path=f'curriculum_schedule_{direction_label}_gradus.pdf')
    ...
    summary_df.to_csv(f'curriculum_summary_{direction_label}_gradus.csv', index=False)
    print(f"Summary saved to curriculum_summary_{direction_label}_gradus.csv")
    ...
    epoch_df.to_csv(f'epoch_history_{direction_label}_gradus.csv', index=False)
    print(f"Epoch history saved to epoch_history_{direction_label}_gradus.csv")
...
def run_cifar10_curriculum_learning():
    ...
    # Load data
    full_train_dataset, val_loader, test_loader, class_names, train_indices = load_images(
        batch_size, val_size=5000
    )
    ...
    features_df.to_csv('image_complexity_features_gradus.csv', index=True)
    print("Saved complexity features to image_complexity_features_gradus.csv")

    plot_complexity_distribution(features_df, save_path='complexity_distribution_gradus.pdf')
    ...
    ranking_df.to_csv('unsupervised_ranking_weights_gradus.csv', index=False)
    print("Saved per-image ranking weights to unsupervised_ranking_weights_gradus.csv")
    ...
    results_t2b = run_single_experiment(
        "top_to_bottom", top_to_bottom, full_train_dataset,
        val_loader, test_loader, class_names, total_train_samples,
        device, total_epochs, batch_size, learning_rate, weight_decay,
        momentum, start_fraction, full_data_epoch
    )
    ...
    results_b2t = run_single_experiment(
        "bottom_to_top", bottom_to_top, full_train_dataset,
        val_loader, test_loader, class_names, total_train_samples,
        device, total_epochs, batch_size, learning_rate, weight_decay,
        momentum, start_fraction, full_data_epoch
    )
    ...
    plot_comparison(results_t2b, results_b2t, save_path='comparison_gradus.pdf')

    print(f"\n{'Direction':<20} {'Best Val Acc':>12} {'Test Acc':>10} {'Train Time':>12}")
    print(f"{'-'*56}")
    print(f"{'Top-to-Bottom':<20} {results_t2b['best_val_acc']:>11.2f}% {results_t2b['final_test_acc']:>9.2f}% {results_t2b['total_training_time']:>10.1f}s")
    print(f"{'Bottom-to-Top':<20} {results_b2t['best_val_acc']:>11.2f}% {results_b2t['final_test_acc']:>9.2f}% {results_b2t['total_training_time']:>10.1f}s")

    # Save combined comparison
    comparison_df = pd.concat([results_t2b['summary_df'], results_b2t['summary_df']], ignore_index=True)
    comparison_df.to_csv('curriculum_comparison_gradus.csv', index=False)
    print("\nComparison saved to curriculum_comparison_gradus.csv")
...
if __name__ == "__main__":
    ...
    model_t2b, model_b2t, comparison_df = run_cifar10_curriculum_learning()
