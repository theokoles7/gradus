...
def compute_all_complexity_features(dataset, device, max_samples=None):
    ...
    Metrics and their complexity direction:
        - color_variance:     higher = more complex (ascending = easy→hard)
        - compression_ratio:  higher = simpler      (descending = easy→hard, inverted for score)
        - edge_density:       higher = more complex (ascending = easy→hard)
        - spatial_frequency:  higher = more complex (ascending = easy→hard)
        - wavelet_energy:     higher = more complex (ascending = easy→hard)
        - wavelet_entropy:    higher = more complex (ascending = easy→hard)
        - edge_object_count:  higher = more complex (ascending = easy→hard)

    Returns:
        features_df: DataFrame with all metric columns + complexity_score
        sorted_indices: indices sorted from least complex to most complex
    """
    ...
    # Sort by complexity: least complex first (ascending)
    sorted_indices = features_df['complexity_score'].argsort().values
...
class CurriculumSampler(Sampler):
    """
    Sampler that introduces training data from easy to hard over the course of training.

    At epoch 0: only the easiest `start_fraction` of data is used.
    Linearly increases to 100% of data by `full_data_epoch`.
    After that, all data is used.

    The curriculum ordering is ALWAYS maintained — no random shuffling at any point.
    Samples are always presented in easy-to-hard order within each epoch.
    """
    def __init__(self, sorted_indices, total_epochs, start_fraction=0.3, full_data_epoch=None):
        """
        Args:
            sorted_indices: array of dataset indices sorted from easiest to hardest
...
def run_cifar10_curriculum_learning():
    ...
    start_fraction = 0.3      # Start with 30% easiest images
    ...
    print(f"\nCurriculum order established: {len(curriculum_order)} training samples")
    ...
    # -------------------------------------------------------
    print("\n" + "=" * 60)
    print("STEP 2: Setting Up Curriculum Learning (No Shuffling)")
    print("=" * 60)
    ...
    # -------------------------------------------------------
    print("\n" + "=" * 60)
    print("STEP 3: Training with Curriculum Learning (No Shuffling)")
    print("=" * 60)
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'best_resnet18_curriculum_gradus.pth')
    ...
    plot_confusion_matrix(test_targets, test_preds, class_names, title='Test Set Confusion Matrix (Curriculum Learning - Gradus)')
    plot_curriculum_schedule(total_epochs, start_fraction, full_data_epoch, total_train_samples, samples_per_epoch)
    ...
    summary_df.to_csv('curriculum_learning_summary_gradus.csv', index=False)
    print("\nSummary saved to curriculum_learning_summary_gradus.csv")
    ...
    epoch_df.to_csv('curriculum_epoch_history_gradus.csv', index=False)
    print("Epoch history saved to curriculum_epoch_history_gradus.csv")
...
if __name__ == "__main__":
    ...
    model, summary_df = run_cifar10_curriculum_learning()
